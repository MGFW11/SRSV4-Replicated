...
local function ForcePart(v)
    if v:IsA("Part") and not v.Anchored and not v.Parent:FindFirstChild("Humanoid") and not v.Parent:FindFirstChild("Head") and v.Name ~= "Handle" then
        ...
        AlignPosition.MaxVelocity = 999999999999999999
        AlignPosition.Responsiveness = 999999999999999999
...
local radius = 50
local height = 100
local rotationSpeed = 55
local attractionStrength = 5500
...
local userId = Players:GetUserIdFromNameAsync("Mhffa11")
local thumbType = Enum.ThumbnailType.HeadShot
local thumbSize = Enum.ThumbnailSize.Size420x420
local content, isReady = Players:GetUserThumbnailAsync(userId, thumbType, thumbSize)

StarterGui:SetCore("SendNotification", {
    Title = "Super ring parts V4 (Replicated)",
    Text = "Enjoy Using",
    Icon = content,
    Duration = 5
})
